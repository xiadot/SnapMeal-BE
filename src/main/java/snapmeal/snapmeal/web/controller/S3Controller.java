...
import snapmeal.snapmeal.web.dto.ClassNameResponseDto;
...
public class S3Controller {
    ...
    @Operation(
            ...
            description = "이미지를 S3에 업로드하고 DB에 URL, user_id를 DB에 저장한 뒤, 예측된 class_name을 반환합니다.",
    ...
    public ResponseEntity<ClassNameResponseDto> uploadImage(
            @RequestPart MultipartFile file
    ) throws IOException {
        // S3 업로드 + 예측
        PredictionResponseDto predictionResult = s3UploadService.uploadPredictAndSave(file);

        // classId -> className 반환
        String className = ClassNameMapper.getClassName(predictionResult.getClassId());
        ClassNameResponseDto classNameResult = new ClassNameResponseDto(className);

        return ResponseEntity.ok(classNameResult);
    }
}